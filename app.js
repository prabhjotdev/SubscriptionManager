...
app.get('/', (req, res) => {
  res.send('Hello, this is your Express server connected to MySQL!');
});
...
app.get('/subscriptions', (req, res) => {
    db.query('SELECT * FROM Subscriptions', (err, results) => {
      ...
      let subs = res.json(results).status(200);
      if (subs > 0 ){
        console.log(subs);
      }else{
        console.log('zero subs added');
      }
    });
  });
  ...
  app.post('/subscription', (req, res) => {
    ...
    // Basic validation
    if (!Name || isNaN(Cost) || Cost <= 0 || !BillingPeriod || !NextBillingDate || !Status) {
    ...
    db.query(`Insert Into Subscriptions (Name, Cost, BillingPeriod, NextBillingDate, Status) VALUES('${Name}',${Cost},'${BillingPeriod}','${formattedDate}','${Status}')`, (err, results) => {
      if (err) {
        console.error('Error executing query:', err);
        res.status(500).send('Internal Server Error');
        return;
